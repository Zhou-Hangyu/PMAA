...
parser.add_argument("--root_file", type=str, 
                    default='/share/hariharan/ck696/allclear/baselines/PMAA/data', 
                    help="Path to dataset")
parser.add_argument("--cloud_model_path", type=str,
                    default='./data/Feature_Extrator_FS2.pth', 
                    help="path to feature extractor model")
parser.add_argument("--save_model_path", type=str,
                    default='./checkpoints_0602', 
                    help="Path to save model")
parser.add_argument("--dataset_name", type=str, 
                    choices=["CTGAN_Sen2_MTC", "AllClear"],
                    default='AllClear', 
                    help="name of the dataset")
parser.add_argument("--load_gen", 
                    type=str, 
                    default='',
                    help="path to the model of generator")
parser.add_argument("--load_dis", 
                    type=str, 
                    default='',
...
parser.add_argument("--resume_at", type=int,
                    default=-1, help="resume training")
...
def load_checkpoint(checkpoint_path, model_GEN, model_DIS, optimizer_G, optimizer_D, scheduler_G, scheduler_D):
    """Load checkpoint"""
    print("=> Loading checkpoint")
    checkpoint = torch.load(checkpoint_path)
    
    model_GEN.load_state_dict(checkpoint['model_GEN_state_dict'])
...
run_name = f"PMAA_{dataset_name_}_lm{int(opt.lambda_L1)}_la{int(opt.lambda_aux)}_bs{opt.batch_size}_lr{opt.lr}_0602_20K_v2"
...
if opt.dataset_name == "AllClear":
    ...
    from allclear.dataset import CRDataset
    from torch.utils.data import DataLoader

    import json
    with open('/share/hariharan/cloud_removal/metadata/v3/s2p_tx3_train_20k_v2.json') as f:        
...
if opt.resume_at >= 0:
    start_epoch = load_checkpoint(f'checkpoint_epoch_{opt.resume_at}.pth.tar', model_GEN, model_DIS, optimizer_G, optimizer_D, scheduler_G, scheduler_D)
...
def train(opt, model_GEN, model_DIS, cloud_detection_model, optimizer_G, optimizer_D, train_loader, val_loader):
    ...
    for epoch in range(opt.n_epochs):
        ...
        pbar = tqdm.tqdm(total=2380 // opt.batch_size+1, ncols=0,
        ...
        for batch_ids, batch in enumerate(train_loader): 
            ...
            # real_A = [[], [], []]
            # real_A[0], real_A[1], real_A[2], real_B, M, M_ours = preprocess_images(opt, batch)
        ...
        if psnr_max < psnr:
            ...
            torch.save(model_GEN.state_dict(), os.path.join(
                opt.save_model_path, run_name, f'EP{epoch}_G.pth'))
            ...
            save_checkpoint(state, 
                            filename=os.path.join(opt.save_model_path, 
                                                  run_name, 
